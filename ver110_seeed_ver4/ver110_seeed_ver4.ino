...
int noteDown_5TE(){
 ...
 /* if (!myFile.open("5TE_DATA.txt", O_CREAT | O_TRUNC | O_RDWR)) {
    error("open failed");
  }
 */   
  ...
  snprintf(timeBuf, sizeof(timeBuf), "%04d-%02d-%02d %02d:%02d",
           ...
           now.hour(), now.minute());
  char dataBuf[50];
  snprintf(dataBuf, sizeof(dataBuf), "%d%d-%f-%f-%f",
           EEPROM.read(SN_ADD1),EEPROM.read(SN_ADD2), TE_measure(raw,1),
           TE_measure(raw,2),TE_measure(raw,2));
  
  
  
  /*if (myFile.write(timeBuf, sizeof(timeBuf)) != sizeof(timeBuf)) {
      error("write failed");
    }
  if (myFile.write(dataBuf, sizeof(dataBuf)) != sizeof(dataBuf)) {
      error("write failed");
    }
  */  
