...
//**************************MCP9808 Temperature********
float temp9808_read(int choice) {
...
void noteDown_TEMP(){
  ...
  if (!myFile.open("MCP_DATA.txt", O_RDWR | O_CREAT | O_AT_END)) {
    sd.errorHalt("opening MCP_DATA.txt for write failed");
  }
  myFile.print(EEPROM.read(SN_ADD1),HEX);
  myFile.print(EEPROM.read(SN_ADD2),HEX);
  myFile.print(",");
  myFile.print(timeBuf); 
  myFile.print(",");
  myFile.print(temp9808_read(0));
  myFile.print(" ");
  myFile.print(temp9808_read(1));
  myFile.print(" ");
  myFile.print(temp9808_read(2));
  myFile.println("");
  myFile.close();
