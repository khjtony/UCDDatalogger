...
SoftwareSerial mySerial2(2, 9); // RX, TX
SoftwareSerial mySerial4(4, 9); // RX, TX
SoftwareSerial mySerial7(7, 9); // RX, TX
SoftwareSerial mySerial8(8, 9); // RX, TX
DS3231 rtc;
Adafruit_ADS1115 ADS1115A(0x49);
...
int noteDown_5TE_cell(SoftwareSerial* mySerial){
  TE_deoverflow();
  mySerial->listen();
  byte income;
  int i=0;
...
int noteDown_5TE(){
  ...
  myFile.print(buf);
  myFile.close();
  noteDown_5TE_cell(&mySerial2);
  noteDown_5TE_cell(&mySerial4);
  noteDown_5TE_cell(&mySerial7);
  noteDown_5TE_cell(&mySerial8);
    if (!myFile.open("5TE_DATA.txt", O_RDWR | O_CREAT | O_AT_END)) {
    sd.errorHalt("opening 5TE.txt for write failed");
  }
  myFile.println("");
